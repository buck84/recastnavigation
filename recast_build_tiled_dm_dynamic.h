#ifndef RECAST_BUILD_TILED_DM_DYNAMIC_H_
#define RECAST_BUILD_TILED_DM_DYNAMIC_H_

#include "recast_build.h"

class RaycastBuildTiledDropMeshDynamic : public RaycastBuild
{
public:
	RaycastBuildTiledDropMeshDynamic();
	~RaycastBuildTiledDropMeshDynamic();

	// mesh生成结束，根据mesh信息初始化各种参数
	virtual int addMeshEnd();
	int build(bool genDrop);
	int buildAllInGame(dtNavMesh*& navmeshBuild);
...
protected:
	void cleanup();
	void buildAllTiles(class dtNavMesh* navmesh, bool genDrop);
...
#endif // RECAST_BUILD_TILED_DM_DYNAMIC_H_
