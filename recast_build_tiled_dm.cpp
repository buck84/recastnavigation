...
RaycastBuildTiledDropMesh::RaycastBuildTiledDropMesh() :
...
void RaycastBuildTiledDropMesh::cleanup()
{
	delete[] m_triareas;
	m_triareas = 0;
...
int RaycastBuildTiledDropMesh::addMeshEnd()
{
	int gw = 0, gh = 0;
...
int RaycastBuildTiledDropMesh::build(bool genDrop)
{
	m_buildMode = BM_BUILD;
...
void RaycastBuildTiledDropMesh::buildAllTiles()
{
	if (!m_geom) return;
	if (!m_navMesh) return;
	...
	{
#ifdef UNITY_EDITOR
		NavMeshBuildStep("buildTileDropMesh", m_navMesh->getMaxTiles(), i);
#endif
		int dataSize = 0;
		unsigned char* data = buildTileDropMesh(i, dataSize, m_navMesh);
		if (data)
		{
			m_navMesh->loadTileDropMesh(i, data, dataSize);
		}
		else
		{
...
int RaycastBuildTiledDropMesh::saveAll(const char* path)
{
...
dtNavMesh* RaycastBuildTiledDropMesh::loadAll(const char* buf, int bufSize)
{
	m_buildMode = BM_LOAD;
	// Read header.
	NavMeshSetHeader header;
...
int RaycastBuildTiledDropMesh::addObstacleCylinder(const float* c, const float r, const float h, unsigned int& ret)
{
	frObstacle* ob = 0;
...
int RaycastBuildTiledDropMesh::addObstacleBox(const float* c, const float* ext, const float rad, unsigned int& ret)
{
	frObstacle* ob = 0;
	if (m_nextFreeObstacle)
	{
...
int RaycastBuildTiledDropMesh::removeObstacle(unsigned int ref)
{
	if (!ref)
		return 1;
...
dtStatus RaycastBuildTiledDropMesh::buildNavMeshTilesAt(const int tx, const int ty, class dtNavMesh* navmesh)
{
	float bmin[3], bmax[3];
...
dtStatus RaycastBuildTiledDropMesh::genHeightField(const int tx, const int ty)
{
	...
	tileMeshInfo* tmi = &m_tileMeshInfos[ty*m_widthTile + tx];
	...
	const float tcs = m_tileWidth;
	m_cfg.bmin[0] = m_BMin[0] + tx*tcs;
	m_cfg.bmin[1] = m_BMin[1];
	m_cfg.bmin[2] = m_BMin[2] + ty*tcs;
	m_cfg.bmax[0] = m_BMin[0] + (tx + 1)*tcs;
	m_cfg.bmax[1] = m_BMax[1];
	m_cfg.bmax[2] = m_BMin[2] + (ty + 1)*tcs;
...
unsigned char* RaycastBuildTiledDropMesh::buildTileMesh(const int tx, const int ty, const float* bmin, const float* bmax, int& dataSize)
{
	tileMeshInfo* tmi;
...
void addDropData(int &index, dtDropData* dropData, int tileId, int polyId, int edgeId, dtPolyRef ref,
	unsigned char bmin, unsigned char bmax, unsigned char lowDrop)
{
	...
	dropData[index].tileId = tileId;
	dropData[index].polyId = polyId;
	dropData[index].edgeId = edgeId;
	dropData[index].ref = ref;
	dropData[index].bmin = bmin;
	dropData[index].bmax = bmax;
	dropData[index].lowDrop = lowDrop;
	index++;
}

unsigned char* RaycastBuildTiledDropMesh::buildTileDropMesh(int index, int& dataSize, class dtNavMesh* navmesh)
{
...
bool RaycastBuildTiledDropMesh::dropNoObstacleHori(float p[4][3], class dtNavMesh* navmesh)
{
...
bool RaycastBuildTiledDropMesh::insectSolid(float* ps, float* pd, int x, int y)
{
	int minx, miny, maxx, maxy;
...
dtObstacleRef RaycastBuildTiledDropMesh::getObstacleRef(const frObstacle* ob) const
{
	if (!ob) return 0;
...
frObstacle* RaycastBuildTiledDropMesh::getObstacleByRef(dtObstacleRef ref)
{
	if (!ref)
		return 0;
...
void RaycastBuildTiledDropMesh::getObstacleBounds(const struct frObstacle* ob, float* bmin, float* bmax) const
{
...
dtStatus RaycastBuildTiledDropMesh::applyObstacle(int numNav, class dtNavMesh** navmeshes, int maxTile)
{
	if (maxTile <= 0)
	{
		maxTile = 99999;
...
int RaycastBuildTiledDropMesh::assureInMap(float& x, float& y, float& z)
{
	if (x < m_BMin[0])
		x = m_BMin[0];
	if (x > m_BMax[0])
